...
def polls_list(request):
    ...
    if not request.user.is_superuser:
        if not hasattr(request.user, 'userprofile'):
            all_polls = all_polls.none()
        else:
            all_polls = all_polls.filter(batch=request.user.userprofile.batch)
    search_term = ''
...
def poll_vote(request, poll_id):
    if not hasattr(request.user, 'userprofile'):
        raise PermissionDenied('You do not have permission in this poll!')
    poll = get_object_or_404(Poll, pk=poll_id, batch=request.user.userprofile.batch)
